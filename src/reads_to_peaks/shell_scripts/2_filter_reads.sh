...
do
    ...
    if [ $FLAG_DUP == true ]
    then
        picard MarkDuplicates -I ${INPUT_PREFIX}_${rep}_unfiltered.bam -O ${INPUT_PREFIX}_${rep}_dup.bam -M ${INPUT_PREFIX}_${rep}_dup_metrics.txt --VALIDATION_STRINGENCY LENIENT --REMOVE_DUPLICATES true --VERBOSITY WARNING
        ...
        samtools view -F 3852 -f 2 -q 30 -L ${ROI_FILE} -o ${INPUT_PREFIX}_${rep}_filtered.bam ${INPUT_PREFIX}_${rep}_dup.bam 
    else
        echo "Duplicates removed by the user"
        samtools view -F 2828 -f 2 -q 30 -L ${ROI_FILE} -o ${INPUT_PREFIX}_${rep}_filtered.bam ${INPUT_PREFIX}_${rep}_unfiltered.bam 
    fi 

done

# merge all replicates into a file and transfer it to the output library: step 3
TMP_PRE=(${BIOL_REPS[@]/#/${INPUT_PREFIX}_})
TMP=${TMP_PRE[@]/%/_filtered.bam}
